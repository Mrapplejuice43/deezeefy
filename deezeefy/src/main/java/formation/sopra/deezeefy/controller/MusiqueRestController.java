package formation.sopra.deezeefy.controller;

import java.lang.reflect.Field;
import java.util.List;
import java.util.Map;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.util.ReflectionUtils;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PatchMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
import org.springframework.web.server.ResponseStatusException;

import com.fasterxml.jackson.annotation.JsonView;

import quest.model.Musique;
import quest.model.Views;
import quest.repository.MusiqueRepository;

@RestController
@RequestMapping("/musique")
public class MusiqueRestController {
	@Autowired
	private MusiqueRepository musiqueRepository;

	@GetMapping("")
	@JsonView(Views.ViewMusique.class)
	public List<Musique> findAll() {
		List<Musique> musiques = musiqueRepository.findAll();

		return musiques;
	}

	@GetMapping("/{id}")
	@JsonView(Views.ViewMusique.class)
	public Musique findById(@PathVariable Integer id) {
		Optional <Musique> optMusique = musiqueRepository.findById(id);
		
		return optMusique.get();
	}
	
	@PostMapping("")
	@JsonView(Views.ViewMusique.class)
	public Musique create(@RequestBody Musique musique) {
		musique = musiqueRepository.save(musique);
		
		return musique;
	}
	
	@PutMapping("/{id}")
	@JsonView(Views.ViewMusique.class)
	public Musique update(@RequestBody Musique musique, @PathVariable Integer id) {
		if(id != musique.getId()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
		}
		
		if(!musiqueRepository.existsById(id)) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND);
		}
		
		musique = musiqueRepository.save(musique);
		
		return musique;
	}
	
	@PatchMapping("/{id}")
	@JsonView(Views.ViewMusique.class)
	public Musique partialUpdate(@PathVariable Integer id, @RequestBody Map<String, Object> fields) {
		Optional<Musique> optMusique = musiqueRepository.findById(id);

		if (optMusique.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
		}

		final Musique musique = optMusique.get();

//		if(fields.containsKey("nom")) {
//			musique.setNom((String) fields.get("nom"));
//		}
//		
//		if(fields.containsKey("prenom")) {
//			musique.setPrenom((String) fields.get("prenom"));
//		}
//
//		if(fields.containsKey("email")) {
//			musique.setEmail((String) fields.get("email"));
//		}

		fields.forEach((key, value) -> {
			Field field = ReflectionUtils.findField(Musique.class, key);
			if (field == null) {
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Propriété : " + key + "non trouvée");
			}
			ReflectionUtils.makeAccessible(field);
			ReflectionUtils.setField(field, musique, value);
		});

		return musiqueRepository.save(musique);
	}
	
	@DeleteMapping("/{id}")
	public void delete(@PathVariable Integer id) {
		musiqueRepository.deleteById(id);
	}
	
}
